...
func (r *CarriersDataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ndu_carriers"
}
...
func (d *CarriersDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	if query.NDUId.IsNull() || query.PortColId.IsNull() || query.LinePTPColId.IsNull() {
		diags.AddError(
			"Error Get Carriers",
			"CarriersDataSource: Could not get Carrier for a ndu. ndu ID or Line PTP Col ID is not specified",
		)
		return
	}
	...
	if query.ColId.IsNull() || strings.Compare(strings.ToUpper(query.ColId.ValueString()), "ALL") == 0 {
		body, err = d.client.ExecuteIPMHttpCommand("GET", "/ndus/"+query.NDUId.ValueString()+"/ports/"+query.PortColId.ValueString()+"/linePtps/"+query.LinePTPColId.ValueString()+"/carriers?content=expanded", nil)
	} else {
		body, err = d.client.ExecuteIPMHttpCommand("GET", "/ndus/"+query.NDUId.ValueString()+"/ports/"+query.PortColId.ValueString()+"/linePtps/"+query.LinePTPColId.ValueString()+"/carriers/"+query.ColId.ValueString()+"?content=expanded", nil)
	}
	if err != nil {
		diags.AddError(
