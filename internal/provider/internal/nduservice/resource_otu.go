...
func (r *OTUResource) Metadata(_ context.Context, req resource.MetadataRequest, resp *resource.MetadataResponse) {
	resp.TypeName = req.ProviderTypeName + "_ndu_otu"
}
...
func (r *OTUResource) update(plan *OTUResourceData, ctx context.Context, diags *diag.Diagnostics) {
	...
	if plan.Href.IsNull() && (plan.ColId.IsNull() || plan.ParentId.IsNull()) && (plan.Identifier.DeviceId.IsNull() || plan.Identifier.ColId.IsNull()) {
		diags.AddError(
			"OTUResource: Error update Port",
			"OTUResource: Could not update Port. Href or Port ColId is not specified.",
		)
		return
	}
	...
	tflog.Debug(ctx, "OTUResource: update ## ", map[string]interface{}{"update Request": updateRequest})
	...
	if len(updateRequest) > 0 {
		...
		if err != nil {
			diags.AddError(
				"OTUResource: update ##: Error update Port",
				"update: Could not Marshal OTUResource, unexpected error: "+err.Error(),
			)
			return
		}
		var body []byte
		if !plan.Href.IsNull() {
			...
			body, err = r.client.ExecuteIPMHttpCommand("PUT", "/ndus/" + plan.ParentId.ValueString() + "/otus/" +  strconv.FormatInt(plan.ColId.ValueInt64(),10), rb)
		} else if !plan.Identifier.DeviceId.IsNull() && !plan.Identifier.ColId.IsNull() {
			body, err = r.client.ExecuteIPMHttpCommand("PUT", "/ndus/" + plan.Identifier.DeviceId.ValueString() + "/otus/" +  plan.Identifier.ColId.ValueString(), rb)
		} else {
			diags.AddError(
				"OTUResource: update ##: Error update porr}",
		...
		if err != nil {
			diags.AddError(
				"OTUResource: update ##: Error Unmarshal response",
				"Update:Could not update OTUResource, unexpected error: "+err.Error(),
			)
			return
		}
	}
...
func (r *OTUResource) read(state *OTUResourceData, ctx context.Context, diags *diag.Diagnostics) {
	...
	if state.Id.IsNull() && state.Href.IsNull() && state.Identifier.Href.IsNull() && state.Identifier.DeviceId.IsNull() && state.Identifier.ColId.IsNull() && state.Identifier.Aid.IsNull() && state.Identifier.Id.IsNull() {
		diags.AddError(
			...
			"OTUResource: Could not read. ParentId and Id and Href are not specified.",
		)
		return
	}
	...
	if !state.Href.IsNull() {
		...
		queryStr = "/ndus/" + state.Identifier.DeviceId.ValueString() + "/otus" + queryStr + "&q={\"id\":\"" + state.Id.ValueString() + "\"}"
	} else if !state.Identifier.Aid.IsNull() {
		queryStr = "/ndus/" + state.Identifier.DeviceId.ValueString() + "/otus" + queryStr + "&q={\"state.otuAid\":\"" + state.Identifier.Aid.ValueString() + "\"}"
	} else if !state.Identifier.Id.IsNull() {
		queryStr = "/ndus/" + state.Identifier.DeviceId.ValueString() + "/otus" + queryStr + "&q={\"id\":\"" + state.Identifier.Id.ValueString() + "\"}"
	} else if !state.Identifier.ColId.IsNull() {
		queryStr = "/ndus/" + state.Identifier.DeviceId.ValueString() + "/otus/" + state.Identifier.ColId.ValueString() + queryStr
	} else {
		queryStr = "/ndus" + state.Identifier.DeviceId.ValueString() + "/otus" + queryStr
	}
...
func (otuData *OTUResourceData) populate(data map[string]interface{}, ctx context.Context, diags *diag.Diagnostics) {
	...
	if config != nil {
		...
		for k, v := range config {
			switch k {
			case "name":
				if !otuData.Config.Name.IsNull() {
					otuData.Config.Name = types.StringValue(v.(string))
				}
			case "fecMode":
				if !otuData.Config.FecMode.IsNull() {
					otuData.Config.FecMode = types.StringValue(v.(string))
				}
			case "diagnostics":
				...
				if diagnostics["facLB"] != nil && !otuData.Config.Diagnostics.FacLB.IsNull() {
					otuData.Config.Diagnostics.FacLB = types.StringValue(v.(string))
				}
				if diagnostics["facLBDuration"] != nil && !otuData.Config.Diagnostics.FacLBDuration.IsNull() {
					otuData.Config.Diagnostics.FacLBDuration = types.Int64Value(int64(v.(float64)))
				}
			}
		}
	}
	...
	tflog.Debug(ctx, "OTUResourceData: read ## ", map[string]interface{}{"otuData": otuData})
}
